...
export function AppShellProvider() {
  ...
  const TUTORIAL_STEPS = useMemo(() => {
    const steps = t('tutorial.steps', { returnObjects: true }) || [];
  ...
  const levelUp = useCallback((newLevel, dropsEarned) => {
    setLevelUpModalConfig({ level: newLevel, drops: dropsEarned });
    setShowLevelUpModal(true);
    changeTab(0);
  }, [changeTab]);

  const newAch = useCallback((newAchs) => {
      ...
      <View style={{ flex: 1, backgroundColor: "white" }} onTouchStart={onTouchStart} onTouchEnd={onTouchEnd}>
        ...
        <View style={[styles.header, { paddingTop: insets.top }]} onLayout={(event) => { const { height } = event.nativeEvent.layout; setHeaderHeight(height); }}>
          ...
          <TouchableOpacity onPress={() => { if (footerRef.current) { footerRef.current.onPress(currentScreenInfo.step == 4 ? 5 : 4); } }} style={{ position: "absolute", zIndex: 5, right: 0, paddingBottom: insets.top, top: insets.top + 5, paddingHorizontal: 20 }}>
            <FontAwesome6 name={currentScreenInfo.step == 4 ? "gear" : "user"} color={theme.colors.text} solid size={screenHeight * 0.025} />
          </TouchableOpacity>
        </View>
        <LinearGradient pointerEvents="none" colors={[theme.colors.background, "transparent"]} locations={[0, 0.5]} style={[styles.bottomfade2, { top: headerHeight }]} />
        <Tabs
        ...
        >
          {SCREENS_CONFIG.map((screen) => (
            <Tabs.Screen
              key={screen.name}
              name={screen.name}
              options={{ title: t(screen.title), step: SCREENS_CONFIG.indexOf(screen) }}
            />
          ))}
        </Tabs>
        {!isKeyboardVisible && (
          <LinearGradient pointerEvents="none" colors={["transparent", theme.colors.background]} locations={[0, 0.5]} style={styles.bottomfade} />
        )}
      </View>
      <TutorialOverlay
